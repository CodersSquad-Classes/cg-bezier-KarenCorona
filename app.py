...
def binomial_coeffs(n):
    coeffs = []
    for k in range(n + 1):
        # Calculating binomial coefficients C(n, k)
        coeff = math.comb(n, k)
        coeffs.append(coeff)
    return coeffs
...
def compute_bezier_point(t, ctrl_points, coeffs):
    ...
    x = 0
    y = 0
    for i, (px, py) in enumerate(ctrl_points):
        bernstein_polynomial = coeffs[i] * (t ** i) * ((1 - t) ** (n - i))
        x += bernstein_polynomial * px
        y += bernstein_polynomial * py
    return [int(x), int(y)]
...
def draw_bezier_curve(debug_image, ctrl_points):
    ...
    # Loop over the range t = 0 to t = 1 to calculate points on the bezier curve
...
def get_bezier_ctrl_points(image, brect, handedness, hand_sign_text, finger_gesture_text):
    ...
    if "Close" in hand_sign_text:
        if len(bezier_ctrl_points) < 4:
            ...
            # Here you could reset the points for the next curve, if necessary.
            pass
